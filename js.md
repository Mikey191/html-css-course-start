...
1. **Инкапсуляция: приватные свойства и методы**
- Создайте класс `BankAccount` со следующими особенностями:
  - Приватное свойство `#balance`, которое инициализируется в конструкторе.
  - Публичные методы `deposit(amount)` и `withdraw(amount)`, которые увеличивают или уменьшают баланс.
  - Геттер `balance`, который возвращает текущий баланс.
- Проверьте работу: `создайте объект`, `пополните счет` и `снимите деньги`.
- Важно: доступ к `#balance` снаружи должен быть невозможен напрямую.


2. **Наследование: класс родитель и дочерний класс**
- Создайте два класса:
  - `Animal` (родительский) с конструктором name и методом `speak()`, который пишет в консоль "Animal speaks".
  - `Dog` (дочерний) от `Animal`, с переопределенным методом `speak()`, который пишет "Dog barks".
- Создайте объекты обоих классов и вызовите `speak()`.

3. **Полиморфизм: массив объектов с переопределенными методами. Продолжите задание №2.**
- Создайте массив creatures, который содержит экземпляры Animal и Dog.
- Пройдитесь по массиву циклом и для каждого объекта вызовите speak().
- Убедитесь, что результат зависит от конкретного объекта.

4. Оператор instanceof
- Создайте объект класса Dog и объект класса Animal.
- Проверьте с помощью instanceof:
  - Является ли объект Dog экземпляром Dog?
  - Является ли он экземпляром Animal?
  - Является ли он экземпляром Object?
- Выведите все результаты в консоль.

5. Инкапсуляция через геттеры и сеттеры
- Создайте класс `User`, у которого:
  - Приватное свойство `#password`.
  - Метод `set password(value)`, который устанавливает пароль только если длина строки больше 5 символов. Если меньше — выводите в консоль `"Password too short!"`.
  - Метод `get password()`, который возвращает строку `"Password is hidden"` (реальный пароль возвращать нельзя).
- Проверьте: попробуйте установить короткий пароль и нормальный пароль, затем получите значение пароля через геттер.

6. **Расширение стандартного класса Array**
- Создайте собственный класс `SuperArray`, который наследуется от `Array`.
- Добавьте метод `sum()`, который возвращает сумму всех чисел в массиве.