...
### 13.5 Псевдоклассы для `<input>`. Использование псевдоклассов для изменения стилей.
...
### 14.1 Стили, применяемые к кнопкам по умолчанию.

- **Стандартная таблица стилей для кнопок из пользовательского агента `Google Chrome`**:
...
<img src="img/lesson14/standart-button.jpg" width="400px">

- **Сброс свойств `-webkit-appearance`, `-moz-appearance`, `appearance`**:
...
<img src="img/lesson14/standart-button-without-apperance.jpg" width="400px">

- **Сброс свойств `border`, `border-radius`, `background`**:
...
<img src="img/lesson14/standart-button-without-border-backgrund.jpg" width="300px">

### 14.2 Базовая стилизация кнопки
...
<img src="img/lesson14/blue-button.jpg" width="300px">

### 14.3 Стилизация при наведении на них мыши и при получении ими фокуса
...
- **Особенность стилизации при наведении и получении фокуса** состоит в том, что важен порядок добавления стилей. Если стиль `hover` добавлять до стиля `focus`, все стили работают правльно. Иногда проблемы возникают, когда порядок стилизации идет наоборот. При щелчке мышью по кнопке, стиль `focus` перестает работать.

- **Правильный порядок стилизации**:
...
### 14.4 Минимальная ширина кнопки

**Для того чтобы кнопка хорошо выглядела, она должна иметь `определённую ширину`**. Важно заранее об этом подумать и принять во внимание длинные и короткие надписи, которые могут содержаться на кнопках. Благодаря свойству `min-width` можно установить минимальную ширину кнопки.
...
### 14.6 Шрифты для кнопок

**Элементы форм, по умолчанию, не наследуют семейство шрифта**, назначенное элементам `<html>` или `<body>`.
Для решения этой проблемы свойство `font-family` должно быть установлено в значение `inherit`.
**Если для кнопки нужен особый шрифт, то можно установить и его**.
...
### 14.7 Стилизация отключённых кнопок

**Для того чтобы указать пользователю на то, что кнопка отключена**, к ней можно **добавить** атрибут `disabled`.
...
### 14.8 Внешний вид указателя мыши при наведении его на кнопку

**Стандартный указатель мыши, наведённый на кнопку, выглядит как стрелка**. Для того чтобы переопределить обычное поведение указателя, **рекомендовано поменять стандартный курсор-стрелку на курсор в виде руки**.
...
### 14.9 Кнопки с обводкой

**Кнопка, которая становится кнопкой с обводкой при наведении на неё мыши**. Стиль кнопки должен включать в себя свойство `border`, в котором, **по умолчанию, используется прозрачный цвет**. Предусмотрев такое свойство, мы делаем так, чтобы **границы кнопки, при наведении на неё мыши, не перерисовывались бы**.
...
### 14.10 Кнопки внутри flexbox-контейнеров

- **По умолчанию каждый `flex-элемент` растягивается в пределах ширины родительского элемента**. Для того чтобы избежать этой неприятности, нужно **настроить свойство `align-self`**:
...
- **Если кнопка помещена во `flex-контейнер`, в котором, по умолчанию, задано свойство `flex-direction: row;` высота кнопки будет подогнана под высоту родительского элемента**. Для того чтобы **исправить эту проблему, нужно использовать свойство кнопки `align-self`** или **свойство flex-контейнера `align-items`**.
...
<img src="img/lesson14/result-practice.jpg" width="500px">

## Урок 15. Стилизация форм.

### 15.1 Зачем нужны формы?

- **Формы являются важным интерактивным элементом веб-страницы, который позволяет пользователям взаимодействовать с сайтом**. Они служат для **отправки данных**, **таких как текстовые сообщения, выборы и файлы**. **Формы играют ключевую роль в создании интерактивных веб-приложений и обеспечивают пользователям возможность**:

  - **Вводить данные**: Пользователи могут вводить текст, выбирать варианты из списка, загружать файлы и выполнять другие действия, которые требуют ввода информации.
  - **Отправлять информацию**: Формы позволяют отправлять введенные данные на сервер для обработки. Это может быть полезно для регистрации, обратной связи, покупок и других действий, где требуется взаимодействие с пользователем.

- **Формы используются для сбора информации от пользователей, что делает их незаменимыми в различных сценариях, таких как**:

  - **Регистрация**: **Создание учетных записей на сайтах**, где пользователи вводят свои данные, такие как имя, адрес электронной почты и пароль.
  - **Обратная связь**: **Позволяют пользователям оставлять отзывы**, комментарии или вопросы, что помогает улучшить качество обслуживания и взаимодействия.
  - **Покупки**: **В интернет-магазинах формы используются для оформления заказов**, где пользователи вводят свои данные для доставки и оплаты.
  - **Опросы и тесты**: **Формы позволяют собирать мнения и результаты тестирования**, что полезно для исследований и анализа.

- **Преимущества использования тега `<form>`**: **Тег `<form>` обеспечивает структурированное представление данных, которые пользователь вводит**. Это позволяет легко обрабатывать эти данные на сервере. Преимущества использования тега `<form>` включают:
  - **Структурированность**: **Тег `<form>` группирует элементы ввода, что позволяет четко организовать данные, которые пользователь отправляет**. Это упрощает обработку данных на сервере, так как все данные, относящиеся к одной форме, находятся в одном месте.
  - **Упрощение взаимодействия**: **Использование форм упрощает взаимодействие с пользователем**. Например, при нажатии кнопки отправки формы браузер автоматически собирает все данные, введенные пользователем, и отправляет их на сервер. Это позволяет разработчикам сосредоточиться на логике обработки данных, а не на механике передачи информации.
  - **Улучшение пользовательского опыта**: **Формы могут быть стилизованы и адаптированы под дизайн сайта, что делает их более привлекательными и удобными для пользователей**. Хорошо оформленные формы с понятными метками и подсказками помогают пользователям быстрее и легче заполнять их.
  - **Поддержка различных методов отправки**: **Тег `<form>` поддерживает различные методы отправки данных, такие как GET и POST**, что позволяет разработчикам выбирать наиболее подходящий способ передачи данных в зависимости от требований приложения.

### 15.2 Основные элементы форм

1. `<input>`

   - **Элемент `<input>` используется для ввода данных**. Он может принимать различные типы данных, такие как текст, пароли, чекбоксы и радиокнопки.

2. `<textarea>`

   - **Элемент `<textarea>` предназначен для многострочного текстового ввода**. Он позволяет пользователям вводить длинные тексты, такие как комментарии или сообщения.

3. `<select>`

   - **Элемент `<select>` используется для создания выпадающих списков**, позволяя пользователям выбирать один или несколько вариантов из предложенных.

4. `<button>`

   - **Элемент `<button>` используется для создания кнопок**, которые могут выполнять действия, такие как отправка формы.

5. `<label>`

   - **Элемент `<label>` связывает текстовые метки с элементами формы**, что улучшает доступность и удобство использования. При клике на текст метки фокус перемещается на связанный элемент формы.

### 15.3 Стилизация форм

**Стилизация форм — это важный аспект веб-разработки, который помогает создать привлекательный и удобный интерфейс для пользователей**. Правильная стилизация делает формы более **интуитивно понятными** и **улучшает пользовательский опыт**.

**Единообразный стиль**: Применение **единообразного стиля для всех элементов формы помогает создать гармоничный интерфейс**. Это включает в себя **использование одинаковых шрифтов, цветов, отступов и границ для всех элементов формы**. Такой подход делает интерфейс более профессиональным и удобным для восприятия.

1. **Текстовые поля и текстовые области**

   - **Текстовые поля и текстовые области можно стилизовать, `изменяя фон`, `границы`, `шрифты` и `отступы`**. Это помогает выделить элементы и сделать их более заметными.
   - **Пример разметки**:
...
2. **Кнопки**
   ...
   - **Кнопки можно стилизовать с использованием псевдоклассов для изменения стилей при наведении и фокусе**. Это помогает сделать интерфейс более интерактивным.
...
3. **Выпадающие списки и чекбоксы**

   - **Выпадающие списки и чекбоксы можно кастомизировать с помощью CSS, чтобы они соответствовали общему стилю формы**.
   - **Пример разметки**:
   ...
   - **Стилизация**:
     ...
     ```css
     select {
       padding: 10px; /* Внутренние отступы для выпадающего списка */
       margin: 5px 0; /* Отступы между элементами */
       border: 1px solid #ccc; /* Светло-серая граница */
       border-radius: 4px; /* Закругленные углы */
       width: 100%; /* Ширина на всю доступную ширину контейнера */
     ...
     input[type="checkbox"] {
       margin-right: 5px; /* Отступ справа для чекбокса */
     }
     ```

4. **Обобщающий пример**

   - В этом примере мы создадим **форму, которая включает все основные элементы: текстовое поле, многострочное текстовое поле, выпадающий список и кнопки**.
   - **Пример разметки**:
   ...
   - **Стилизация**:
     ...
     /* Стили при фокусе на текстовых полях и текстовой области */
     input:focus,
     textarea:focus,
     select:focus {
       border-color: #007bff; /* Изменение цвета границы при фокусе */
     ...
     button {
       padding: 10px 15px; /* Внутренние отступы для кнопок */
       margin: 10px 5px 0; /* Отступы между кнопками */
       border: none; /* Убираем стандартную границу */
       border-radius: 4px; /* Закругленные углы */
       background-color: #007bff; /* Синий фон */
       color: white; /* Белый текст */
